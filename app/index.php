...
<body>
  ...
  <main>
    ...
    <?php if (a12CurrentRole() === 'admin'): ?>
      <section id="system" class="view">
        <div class="sectionIntro"><p>Versionsstand, Updates, Datensicherung und administrative Wartung dieser Installation.</p></div>
        <div class="systemGrid">
          <article class="systemCard"><span class="systemLabel">INSTALLIERTE VERSION</span><strong class="systemVersion">v<?= htmlspecialchars(A12_APP_VERSION, ENT_QUOTES, 'UTF-8') ?></strong><small>A12-Teilchenbeschleuniger</small></article>
          <article class="systemCard updateCard">
            <div class="systemCardHead"><div><span class="systemLabel">GITHUB UPDATES</span><h2 id="updateHeadline">Update wird geprüft …</h2></div><button class="iconbtn" type="button" id="checkUpdateBtn" title="Jetzt prüfen">↻</button></div>
            <p id="updateMessage">Verbindung zum Release-Kanal wird hergestellt.</p><div class="releaseNotes" id="releaseNotes" hidden></div>
            <div class="updateMeta"><span id="updateChecked"></span><span class="updateLinks"><a href="https://github.com/DL1DRK/a12-teilchenbeschleuniger/blob/main/CHANGELOG.md" target="_blank" rel="noopener">Changelog ↗</a><a href="https://github.com/DL1DRK/a12-teilchenbeschleuniger" target="_blank" rel="noopener">GitHub Repository ↗</a></span></div>
            <div class="updateActions" id="updateActions" hidden><a class="secondaryButton" id="releaseLink" target="_blank" rel="noopener">Release-Details</a><a class="secondaryButton" id="updaterLink" target="_blank" rel="noopener">Manuell herunterladen</a><button class="primary" type="button" id="prepareUpdateBtn" hidden>Update vorbereiten</button></div>
            <small class="updateHint">„Update vorbereiten“ lädt und prüft den Updater. Installiert wird erst nach Ihrer Bestätigung auf der nächsten Seite.</small>
          </article>
        </div>
        <div class="maintenanceGrid">
          <article class="systemCard maintenanceCard">
            <span class="systemLabel">DATENSICHERUNG</span><h2>Backup &amp; Wiederherstellung</h2>
            <p>Das Vollbackup enthält Bauteile, Bewegungen, Benutzerkonten, Passwort-Hashes und Einstellungen.</p>
            <div class="maintenanceActions"><button class="primary" type="button" id="backupBtn">Backup herunterladen</button><button class="secondaryButton" type="button" id="restoreOpenBtn">Backup einspielen</button></div>
            <small>Backupdateien enthalten vertrauliche Anmeldedaten und sollten geschützt aufbewahrt werden.</small>
          </article>
          <article class="systemCard maintenanceCard dangerZone">
            <span class="systemLabel">GEFAHRENZONE</span><h2>System zurücksetzen</h2>
            <p>Interne Nummern neu aufbauen oder Bestand, Bewegungen und weitere Benutzer vollständig entfernen.</p>
            <div class="maintenanceActions"><button class="dangerOutline" type="button" id="resetOpenBtn">Reset vorbereiten</button></div>
            <small>Jeder Reset erfordert das aktuelle Administratorpasswort in doppelter Eingabe.</small>
          </article>
        </div>
      </section>
    <?php endif; ?>
  ...
  <?php if (a12CurrentRole() === 'admin'): ?>
    <dialog id="restoreDialog"><form id="restoreForm" enctype="multipart/form-data"><div class="dialogHead"><div><p class="eyebrow">DATENSICHERUNG</p><h2>Backup einspielen</h2></div><button class="close" type="button" data-close-dialog="restoreDialog">×</button></div><div class="restoreWarning"><strong>Vorhandene Daten werden ersetzt.</strong><br>Das Backup muss mindestens ein aktives Administratorkonto enthalten. Fehlt Ihr aktueller Benutzer im Backup, werden Sie nach dem Einspielen abgemeldet.</div><div class="formgrid"><label class="wide">Backupdatei *<input name="backup" type="file" accept="application/json,.json" required></label><label>Aktuelles Passwort *<input name="password" type="password" autocomplete="current-password" required></label><label>Passwort wiederholen *<input name="password_confirm" type="password" autocomplete="current-password" required></label></div><div class="dialogActions"><button type="button" data-close-dialog="restoreDialog">Abbrechen</button><button class="primary" id="restoreBtn" type="submit">Backup einspielen</button></div></form></dialog>
