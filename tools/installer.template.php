...
const A12_INSTALLER_VERSION = '2.3.0';
    ...
    if (!$errors) {
        try {
            ...
            if (is_file($installedPath . DIRECTORY_SEPARATOR . 'config.php')) {
                throw new RuntimeException('Im Zielordner befindet sich bereits eine konfigurierte A12-Installation. Dieser Installer führt bewusst kein Datenbank-Upgrade durch.');
            }
  ...
  <section class="card">
    ...
    <div class="content">
      <?php if ($success): ?>
        ...
        <div class="action"><a class="button" href="<?= h($installedUrl) ?>">A12 öffnen →</a></div>
      <?php elseif ($isLocked): ?>
      ...
      <?php else: ?>
        ...
        <form method="post">
          ...
          <div class="action"><button class="button" type="submit" <?= $requirementsOkay ? '' : 'disabled' ?>>A12 installieren</button></div>
        </form>
      <?php endif; ?>
    </div>
  </section>
